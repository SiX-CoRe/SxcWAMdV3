...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    if (id in conn['tebakgambar'] || id in conn.game) {
        conn.reply(m.chat, `⚠️ Masih ada game *${conn.game[id] || 'tebakgambar'}* yang belum terjawab di chat ini!\nSelesaikan atau ketik *nyerah* terlebih dahulu.`, conn['tebakgambar']?.[id]?.[0] || m);
        throw false;
    }
    ...
    try {
        ...
        const response = await fetch(`${global.web}/api/game/tebakgambar?apikey=${apiKey}`);
        ...
        console.error('[Game tebakgambar Error]', e);
