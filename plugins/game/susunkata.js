...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    if (id in conn['susunkata'] || id in conn.game) {
        conn.reply(m.chat, `⚠️ Masih ada game *${conn.game[id] || 'susunkata'}* yang belum terjawab di chat ini!\nSelesaikan atau ketik *nyerah* terlebih dahulu.`, conn['susunkata']?.[id]?.[0] || m);
        throw false;
    }
    ...
    try {
        ...
        const response = await fetch(`${global.web}/api/game/susunkata?apikey=${apiKey}`);
        ...
        console.error('[Game susunkata Error]', e);
