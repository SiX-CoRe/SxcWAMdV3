...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    if (id in conn['tebakhewan'] || id in conn.game) {
        conn.reply(m.chat, `⚠️ Masih ada game *${conn.game[id] || 'tebakhewan'}* yang belum terjawab di chat ini!\nSelesaikan atau ketik *nyerah* terlebih dahulu.`, conn['tebakhewan']?.[id]?.[0] || m);
        throw false;
    }
    ...
    try {
        ...
        const response = await fetch(`${global.web}/api/game/tebakhewan?apikey=${apiKey}`);
        ...
        console.error('[Game tebakhewan Error]', e);
