...
handler.before = async function (m) {
    ...
    if (this.game[id] === 'tebakheroml' && id in this['tebakheroml']) {
        if (!m.text) return !0;

        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text.trim());
        if (isSurrender) {
            clearTimeout(this['tebakheroml'][id][2]);
            let ans = this['tebakheroml'][id][3] || this['tebakheroml'][id][1]?.result?.jawaban;
        ...
        let rawAns = this['tebakheroml'][id][3] || this['tebakheroml'][id][1]?.result?.jawaban;
        ...
        if (isCorrect) {
            ...
            let extraDesc = this['tebakheroml'][id][1]?.result?.deskripsi ? `\n\n📝 *Penjelasan:* ${this['tebakheroml'][id][1].result.deskripsi}` : '';
            ...
            let isReply = m.quoted && m.quoted.id === this['tebakheroml'][id][0]?.key?.id;
            if (isReply) {
