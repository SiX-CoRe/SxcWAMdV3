...
handler.before = async function (m) {
    ...
    if (this.game[id] === 'tebakhewan' && id in this['tebakhewan']) {
        if (!m.text) return !0;

        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text.trim());
        if (isSurrender) {
            clearTimeout(this['tebakhewan'][id][2]);
            let ans = this['tebakhewan'][id][3] || this['tebakhewan'][id][1]?.result?.jawaban;
        ...
        let rawAns = this['tebakhewan'][id][3] || this['tebakhewan'][id][1]?.result?.jawaban;
        ...
        if (isCorrect) {
            ...
            let extraDesc = this['tebakhewan'][id][1]?.result?.deskripsi ? `\n\n📝 *Penjelasan:* ${this['tebakhewan'][id][1].result.deskripsi}` : '';
            ...
            let isReply = m.quoted && m.quoted.id === this['tebakhewan'][id][0]?.key?.id;
            if (isReply) {
