...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    if (id in conn['caklontong'] || id in conn.game) {
        conn.reply(m.chat, `⚠️ Masih ada game *${conn.game[id] || 'caklontong'}* yang belum terjawab di chat ini!\nSelesaikan atau ketik *nyerah* terlebih dahulu.`, conn['caklontong']?.[id]?.[0] || m);
        throw false;
    }
    ...
    try {
        ...
        const response = await fetch(`${global.web}/api/game/caklontong?apikey=${apiKey}`);
        ...
        console.error('[Game caklontong Error]', e);
