...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    if (id in conn['tebakwarna'] || id in conn.game) {
        conn.reply(m.chat, `⚠️ Masih ada game *${conn.game[id] || 'tebakwarna'}* yang belum terjawab di chat ini!\nSelesaikan atau ketik *nyerah* terlebih dahulu.`, conn['tebakwarna']?.[id]?.[0] || m);
        throw false;
    }
    ...
    try {
        ...
        const response = await fetch(`${global.web}/api/game/tebakwarna?apikey=${apiKey}`);
        ...
        console.error('[Game tebakwarna Error]', e);
