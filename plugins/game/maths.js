...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    conn['maths'] = conn['maths'] ? conn['maths'] : {};
    let id = m.chat;

    if (id in conn['maths'] || id in conn.game) {
        conn.reply(m.chat, `⚠️ Masih ada game *${conn.game[id] || 'maths'}* yang belum terjawab di chat ini!\nSelesaikan atau ketik *nyerah* terlebih dahulu.`, conn['maths']?.[id]?.[0] || m);
        throw false;
    }
    ...
    try {
        ...
        conn['maths'][id] = [
            ...
            json,
            setTimeout(() => {
                if (conn['maths'] && conn['maths'][id]) {
                    let ansDisplay = Array.isArray(answerData) ? answerData.join(' / ') : answerData;
                    conn.reply(m.chat, `⏳ *WAKTU HABIS!*\n\nJawabannya adalah: *${ansDisplay}*`, conn['maths'][id][0]);
                    delete conn['maths'][id];
        ...
        console.error('[Game maths Error]', e);
