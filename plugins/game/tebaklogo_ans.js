...
handler.before = async function (m) {
    ...
    if (this.game[id] === 'tebaklogo' && id in this['tebaklogo']) {
        if (!m.text) return !0;

        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text.trim());
        if (isSurrender) {
            clearTimeout(this['tebaklogo'][id][2]);
            let ans = this['tebaklogo'][id][3] || this['tebaklogo'][id][1]?.result?.jawaban;
        ...
        let rawAns = this['tebaklogo'][id][3] || this['tebaklogo'][id][1]?.result?.jawaban;
        ...
        if (isCorrect) {
            ...
            let extraDesc = this['tebaklogo'][id][1]?.result?.deskripsi ? `\n\n📝 *Penjelasan:* ${this['tebaklogo'][id][1].result.deskripsi}` : '';
            ...
            let isReply = m.quoted && m.quoted.id === this['tebaklogo'][id][0]?.key?.id;
            if (isReply) {
