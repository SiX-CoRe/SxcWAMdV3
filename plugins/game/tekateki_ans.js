...
handler.before = async function (m) {
    ...
    if (this.game[id] === 'tekateki' && id in this['tekateki']) {
        if (!m.text) return !0;

        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text.trim());
        if (isSurrender) {
            clearTimeout(this['tekateki'][id][2]);
            let ans = this['tekateki'][id][3] || this['tekateki'][id][1]?.result?.jawaban;
        ...
        let rawAns = this['tekateki'][id][3] || this['tekateki'][id][1]?.result?.jawaban;
        ...
        if (isCorrect) {
            ...
            let extraDesc = this['tekateki'][id][1]?.result?.deskripsi ? `\n\n📝 *Penjelasan:* ${this['tekateki'][id][1].result.deskripsi}` : '';
            ...
            let isReply = m.quoted && m.quoted.id === this['tekateki'][id][0]?.key?.id;
            if (isReply) {
