...
handler.before = async function (m) {
    ...
    if (this.game[id] === 'tebaklagu' && id in this['tebaklagu']) {
        if (!m.text) return !0;

        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text.trim());
        if (isSurrender) {
            clearTimeout(this['tebaklagu'][id][2]);
            let ans = this['tebaklagu'][id][3] || this['tebaklagu'][id][1]?.result?.jawaban;
        ...
        let rawAns = this['tebaklagu'][id][3] || this['tebaklagu'][id][1]?.result?.jawaban;
        ...
        if (isCorrect) {
            ...
            let extraDesc = this['tebaklagu'][id][1]?.result?.deskripsi ? `\n\n📝 *Penjelasan:* ${this['tebaklagu'][id][1].result.deskripsi}` : '';
            ...
            let isReply = m.quoted && m.quoted.id === this['tebaklagu'][id][0]?.key?.id;
            if (isReply) {
