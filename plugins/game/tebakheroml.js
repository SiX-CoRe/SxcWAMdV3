...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    if (id in conn['tebakheroml'] || id in conn.game) {
        conn.reply(m.chat, `⚠️ Masih ada game *${conn.game[id] || 'tebakheroml'}* yang belum terjawab di chat ini!\nSelesaikan atau ketik *nyerah* terlebih dahulu.`, conn['tebakheroml']?.[id]?.[0] || m);
        throw false;
    }
    ...
    try {
        ...
        const response = await fetch(`${global.web}/api/game/tebakheroml?apikey=${apiKey}`);
        ...
        let text = `🎮 *${'TEBAK HERO MOBILE LEGENDS'}*\n\n`;
        text += `📝 *Soal:* ${soalText}\n`;
        ...
        console.error('[Game tebakheroml Error]', e);
