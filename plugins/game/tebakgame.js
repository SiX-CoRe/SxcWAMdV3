...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    if (id in conn['tebakgame'] || id in conn.game) {
        conn.reply(m.chat, `⚠️ Masih ada game *${conn.game[id] || 'tebakgame'}* yang belum terjawab di chat ini!\nSelesaikan atau ketik *nyerah* terlebih dahulu.`, conn['tebakgame']?.[id]?.[0] || m);
        throw false;
    }
    ...
    try {
        ...
        const response = await fetch(`${global.web}/api/game/tebakgame?apikey=${apiKey}`);
        ...
        console.error('[Game tebakgame Error]', e);
