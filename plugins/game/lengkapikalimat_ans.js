...
handler.before = async function (m) {
    ...
    if (this.game[id] === 'lengkapikalimat' && id in this['lengkapikalimat']) {
        if (!m.text) return !0;

        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text.trim());
        if (isSurrender) {
            clearTimeout(this['lengkapikalimat'][id][2]);
            let ans = this['lengkapikalimat'][id][3] || this['lengkapikalimat'][id][1]?.result?.jawaban;
        ...
        let rawAns = this['lengkapikalimat'][id][3] || this['lengkapikalimat'][id][1]?.result?.jawaban;
        ...
        if (isCorrect) {
            ...
            let extraDesc = this['lengkapikalimat'][id][1]?.result?.deskripsi ? `\n\n📝 *Penjelasan:* ${this['lengkapikalimat'][id][1].result.deskripsi}` : '';
            ...
            let isReply = m.quoted && m.quoted.id === this['lengkapikalimat'][id][0]?.key?.id;
            if (isReply) {
