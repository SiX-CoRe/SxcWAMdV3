...
handler.before = async function (m) {
    ...
    if (this.game[id] === 'family100' && id in this['family100']) {
        if (!m.text) return !0;

        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text.trim());
        if (isSurrender) {
            clearTimeout(this['family100'][id][2]);
            let ans = this['family100'][id][3] || this['family100'][id][1]?.result?.jawaban;
        ...
        let rawAns = this['family100'][id][3] || this['family100'][id][1]?.result?.jawaban;
        ...
        if (isCorrect) {
            ...
            let extraDesc = this['family100'][id][1]?.result?.deskripsi ? `\n\n📝 *Penjelasan:* ${this['family100'][id][1].result.deskripsi}` : '';
            ...
            let isReply = m.quoted && m.quoted.id === this['family100'][id][0]?.key?.id;
            if (isReply) {
