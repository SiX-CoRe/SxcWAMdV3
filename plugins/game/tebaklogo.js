...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    if (id in conn['tebaklogo'] || id in conn.game) {
        conn.reply(m.chat, `⚠️ Masih ada game *${conn.game[id] || 'tebaklogo'}* yang belum terjawab di chat ini!\nSelesaikan atau ketik *nyerah* terlebih dahulu.`, conn['tebaklogo']?.[id]?.[0] || m);
        throw false;
    }
    ...
    try {
        ...
        const response = await fetch(`${global.web}/api/game/tebaklogo?apikey=${apiKey}`);
        ...
        console.error('[Game tebaklogo Error]', e);
