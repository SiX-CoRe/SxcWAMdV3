...
handler.before = async function (m) {
    ...
    if (this.game[id] === 'tebakgame' && id in this['tebakgame']) {
        if (!m.text) return !0;

        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text.trim());
        if (isSurrender) {
            clearTimeout(this['tebakgame'][id][2]);
            let ans = this['tebakgame'][id][3] || this['tebakgame'][id][1]?.result?.jawaban;
        ...
        let rawAns = this['tebakgame'][id][3] || this['tebakgame'][id][1]?.result?.jawaban;
        ...
        if (isCorrect) {
            ...
            let extraDesc = this['tebakgame'][id][1]?.result?.deskripsi ? `\n\n📝 *Penjelasan:* ${this['tebakgame'][id][1].result.deskripsi}` : '';
            ...
            let isReply = m.quoted && m.quoted.id === this['tebakgame'][id][0]?.key?.id;
            if (isReply) {
