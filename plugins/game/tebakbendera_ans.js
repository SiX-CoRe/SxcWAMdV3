...
handler.before = async function (m) {
    ...
    if (this.game[id] === 'tebakbendera' && id in this['tebakbendera']) {
        if (!m.text) return !0;

        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text.trim());
        if (isSurrender) {
            clearTimeout(this['tebakbendera'][id][2]);
            let ans = this['tebakbendera'][id][3] || this['tebakbendera'][id][1]?.result?.jawaban;
        ...
        let rawAns = this['tebakbendera'][id][3] || this['tebakbendera'][id][1]?.result?.jawaban;
        ...
        if (isCorrect) {
            ...
            let extraDesc = this['tebakbendera'][id][1]?.result?.deskripsi ? `\n\n📝 *Penjelasan:* ${this['tebakbendera'][id][1].result.deskripsi}` : '';
            ...
            let isReply = m.quoted && m.quoted.id === this['tebakbendera'][id][0]?.key?.id;
            if (isReply) {
