...
handler.before = async function (m) {
    ...
    if (this.game[id] === 'caklontong' && id in this['caklontong']) {
        if (!m.text) return !0;

        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text.trim());
        if (isSurrender) {
            clearTimeout(this['caklontong'][id][2]);
            let ans = this['caklontong'][id][3] || this['caklontong'][id][1]?.result?.jawaban;
        ...
        let rawAns = this['caklontong'][id][3] || this['caklontong'][id][1]?.result?.jawaban;
        ...
        if (isCorrect) {
            ...
            let extraDesc = this['caklontong'][id][1]?.result?.deskripsi ? `\n\n📝 *Penjelasan:* ${this['caklontong'][id][1].result.deskripsi}` : '';
            ...
            let isReply = m.quoted && m.quoted.id === this['caklontong'][id][0]?.key?.id;
            if (isReply) {
