...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    if (id in conn['tebakkimia'] || id in conn.game) {
        conn.reply(m.chat, `⚠️ Masih ada game *${conn.game[id] || 'tebakkimia'}* yang belum terjawab di chat ini!\nSelesaikan atau ketik *nyerah* terlebih dahulu.`, conn['tebakkimia']?.[id]?.[0] || m);
        throw false;
    }
    ...
    try {
        ...
        const response = await fetch(`${global.web}/api/game/tebakkimia?apikey=${apiKey}`);
        ...
        console.error('[Game tebakkimia Error]', e);
