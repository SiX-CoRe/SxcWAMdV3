...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    if (id in conn['tekateki'] || id in conn.game) {
        conn.reply(m.chat, `⚠️ Masih ada game *${conn.game[id] || 'tekateki'}* yang belum terjawab di chat ini!\nSelesaikan atau ketik *nyerah* terlebih dahulu.`, conn['tekateki']?.[id]?.[0] || m);
        throw false;
    }
    ...
    try {
        ...
        const response = await fetch(`${global.web}/api/game/tekateki?apikey=${apiKey}`);
        ...
        console.error('[Game tekateki Error]', e);
