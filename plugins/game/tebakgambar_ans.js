...
handler.before = async function (m) {
    ...
    if (this.game[id] === 'tebakgambar' && id in this['tebakgambar']) {
        if (!m.text) return !0;

        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text.trim());
        if (isSurrender) {
            clearTimeout(this['tebakgambar'][id][2]);
            let ans = this['tebakgambar'][id][3] || this['tebakgambar'][id][1]?.result?.jawaban;
        ...
        let rawAns = this['tebakgambar'][id][3] || this['tebakgambar'][id][1]?.result?.jawaban;
        ...
        if (isCorrect) {
            ...
            let extraDesc = this['tebakgambar'][id][1]?.result?.deskripsi ? `\n\n📝 *Penjelasan:* ${this['tebakgambar'][id][1].result.deskripsi}` : '';
            ...
            let isReply = m.quoted && m.quoted.id === this['tebakgambar'][id][0]?.key?.id;
            if (isReply) {
