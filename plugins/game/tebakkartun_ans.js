...
handler.before = async function (m) {
    ...
    if (this.game[id] === 'tebakkartun' && id in this['tebakkartun']) {
        if (!m.text) return !0;

        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text.trim());
        if (isSurrender) {
            clearTimeout(this['tebakkartun'][id][2]);
            let ans = this['tebakkartun'][id][3] || this['tebakkartun'][id][1]?.result?.jawaban;
        ...
        let rawAns = this['tebakkartun'][id][3] || this['tebakkartun'][id][1]?.result?.jawaban;
        ...
        if (isCorrect) {
            ...
            let extraDesc = this['tebakkartun'][id][1]?.result?.deskripsi ? `\n\n📝 *Penjelasan:* ${this['tebakkartun'][id][1].result.deskripsi}` : '';
            ...
            let isReply = m.quoted && m.quoted.id === this['tebakkartun'][id][0]?.key?.id;
            if (isReply) {
