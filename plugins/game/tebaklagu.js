...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    if (id in conn['tebaklagu'] || id in conn.game) {
        conn.reply(m.chat, `⚠️ Masih ada game *${conn.game[id] || 'tebaklagu'}* yang belum terjawab di chat ini!\nSelesaikan atau ketik *nyerah* terlebih dahulu.`, conn['tebaklagu']?.[id]?.[0] || m);
        throw false;
    }
    ...
    try {
        ...
        const response = await fetch(`${global.web}/api/game/tebaklagu?apikey=${apiKey}`);
        ...
        let isAudio = 'audio' === 'audio' || (mediaUrl && (mediaUrl.endsWith('.mp3') || mediaUrl.endsWith('.opus') || mediaUrl.endsWith('.m4a')));

        let text = `🎮 *${'TEBAK JUDUL LAGU'}*\n\n`;
        text += `📝 *Soal:* ${soalText}\n`;
        ...
        console.error('[Game tebaklagu Error]', e);
