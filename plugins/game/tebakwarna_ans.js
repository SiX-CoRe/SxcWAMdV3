...
handler.before = async function (m) {
    ...
    this['tebakwarna'] = this['tebakwarna'] ? this['tebakwarna'] : {};

    if (this.game[id] === 'tebakwarna' && id in this['tebakwarna']) {
        if (!m.text) return !0;

        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text.trim());
        if (isSurrender) {
            clearTimeout(this['tebakwarna'][id][2]);
            let ans = this['tebakwarna'][id][3] || this['tebakwarna'][id][1]?.result?.jawaban;
            let ansDisplay = Array.isArray(ans) ? ans.join(' / ') : ans;
            delete this['tebakwarna'][id];
            delete this.game[id];
            return m.reply(`🏳️ *Kamu Menyerah!*\n\nJawabannya adalah: *${ansDisplay}*`);
        }

        let rawAns = this['tebakwarna'][id][3] || this['tebakwarna'][id][1]?.result?.jawaban;
        ...
        if (isCorrect) {
            ...
            let extraDesc = this['tebakwarna'][id][1]?.result?.deskripsi ? `\n\n📝 *Penjelasan:* ${this['tebakwarna'][id][1].result.deskripsi}` : '';
            ...
            let isReply = m.quoted && m.quoted.id === this['tebakwarna'][id][0]?.key?.id;
            if (isReply) {
