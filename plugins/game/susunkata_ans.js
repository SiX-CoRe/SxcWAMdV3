...
handler.before = async function (m) {
    ...
    if (this.game[id] === 'susunkata' && id in this['susunkata']) {
        if (!m.text) return !0;

        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text.trim());
        if (isSurrender) {
            clearTimeout(this['susunkata'][id][2]);
            let ans = this['susunkata'][id][3] || this['susunkata'][id][1]?.result?.jawaban;
        ...
        let rawAns = this['susunkata'][id][3] || this['susunkata'][id][1]?.result?.jawaban;
        ...
        if (isCorrect) {
            ...
            let extraDesc = this['susunkata'][id][1]?.result?.deskripsi ? `\n\n📝 *Penjelasan:* ${this['susunkata'][id][1].result.deskripsi}` : '';
            ...
            let isReply = m.quoted && m.quoted.id === this['susunkata'][id][0]?.key?.id;
            if (isReply) {
