...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    if (id in conn['tebakkalimat'] || id in conn.game) {
        conn.reply(m.chat, `⚠️ Masih ada game *${conn.game[id] || 'tebakkalimat'}* yang belum terjawab di chat ini!\nSelesaikan atau ketik *nyerah* terlebih dahulu.`, conn['tebakkalimat']?.[id]?.[0] || m);
        throw false;
    }
    ...
    try {
        ...
        const response = await fetch(`${global.web}/api/game/tebakkalimat?apikey=${apiKey}`);
        ...
        console.error('[Game tebakkalimat Error]', e);
