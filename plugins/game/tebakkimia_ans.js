...
handler.before = async function (m) {
    ...
    if (this.game[id] === 'tebakkimia' && id in this['tebakkimia']) {
        if (!m.text) return !0;

        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text.trim());
        if (isSurrender) {
            clearTimeout(this['tebakkimia'][id][2]);
            let ans = this['tebakkimia'][id][3] || this['tebakkimia'][id][1]?.result?.jawaban;
        ...
        let rawAns = this['tebakkimia'][id][3] || this['tebakkimia'][id][1]?.result?.jawaban;
        ...
        if (isCorrect) {
            ...
            let extraDesc = this['tebakkimia'][id][1]?.result?.deskripsi ? `\n\n📝 *Penjelasan:* ${this['tebakkimia'][id][1].result.deskripsi}` : '';
            ...
            let isReply = m.quoted && m.quoted.id === this['tebakkimia'][id][0]?.key?.id;
            if (isReply) {
