...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    conn['karakterff'] = conn['karakterff'] ? conn['karakterff'] : {};
    let id = m.chat;

    if (id in conn['karakterff'] || id in conn.game) {
        conn.reply(m.chat, `⚠️ Masih ada game *${conn.game[id] || 'karakterff'}* yang belum terjawab di chat ini!\nSelesaikan atau ketik *nyerah* terlebih dahulu.`, conn['karakterff']?.[id]?.[0] || m);
        throw false;
    }
    ...
    try {
        ...
        const response = await fetch(`${global.web}/api/game/karakterff?apikey=${apiKey}`);
        ...
        let text = `🎮 *${'TEBAK KARAKTER FREE FIRE'}*\n\n`;
        text += `📝 *Soal:* ${soalText}\n`;
        ...
        conn.game[id] = 'karakterff';
        conn['karakterff'][id] = [
            await conn.sendMessage(m.chat, msgOptions, { quoted: m }),
            json,
            setTimeout(() => {
                if (conn['karakterff'] && conn['karakterff'][id]) {
                    let ansDisplay = Array.isArray(answerData) ? answerData.join(' / ') : answerData;
                    conn.reply(m.chat, `⏳ *WAKTU HABIS!*\n\nJawabannya adalah: *${ansDisplay}*`, conn['karakterff'][id][0]);
                    delete conn['karakterff'][id];
        ...
        console.error('[Game karakterff Error]', e);
        m.reply("❌ Error: " + (e.message || "Gagal memulai game"));
    }
}

handler.help = ['karakterff']
handler.tags = ['game']
handler.command = /^karakterff$/i
handler.limit = 1;
