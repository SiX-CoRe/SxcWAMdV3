...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    if (id in conn['tebakkata'] || id in conn.game) {
        conn.reply(m.chat, `⚠️ Masih ada game *${conn.game[id] || 'tebakkata'}* yang belum terjawab di chat ini!\nSelesaikan atau ketik *nyerah* terlebih dahulu.`, conn['tebakkata']?.[id]?.[0] || m);
        throw false;
    }
    ...
    try {
        ...
        const response = await fetch(`${global.web}/api/game/tebakkata?apikey=${apiKey}`);
        ...
        console.error('[Game tebakkata Error]', e);
