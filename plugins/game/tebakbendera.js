...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    if (id in conn['tebakbendera'] || id in conn.game) {
        conn.reply(m.chat, `⚠️ Masih ada game *${conn.game[id] || 'tebakbendera'}* yang belum terjawab di chat ini!\nSelesaikan atau ketik *nyerah* terlebih dahulu.`, conn['tebakbendera']?.[id]?.[0] || m);
        throw false;
    }
    ...
    try {
        ...
        const response = await fetch(`${global.web}/api/game/tebakbendera?apikey=${apiKey}`);
        ...
        console.error('[Game tebakbendera Error]', e);
