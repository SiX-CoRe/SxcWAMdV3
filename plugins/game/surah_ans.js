...
handler.before = async function (m) {
    ...
    this['surah'] = this['surah'] ? this['surah'] : {};

    if (this.game[id] === 'surah' && id in this['surah']) {
        if (!m.text) return !0;

        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text.trim());
        if (isSurrender) {
            clearTimeout(this['surah'][id][2]);
            let ans = this['surah'][id][3] || this['surah'][id][1]?.result?.jawaban;
            let ansDisplay = Array.isArray(ans) ? ans.join(' / ') : ans;
            delete this['surah'][id];
            delete this.game[id];
            return m.reply(`🏳️ *Kamu Menyerah!*\n\nJawabannya adalah: *${ansDisplay}*`);
        }

        let rawAns = this['surah'][id][3] || this['surah'][id][1]?.result?.jawaban;
        ...
        if (isCorrect) {
            ...
            let extraDesc = this['surah'][id][1]?.result?.deskripsi ? `\n\n📝 *Penjelasan:* ${this['surah'][id][1].result.deskripsi}` : '';
            ...
            let isReply = m.quoted && m.quoted.id === this['surah'][id][0]?.key?.id;
            if (isReply) {
