...
handler.before = async function (m) {
    ...
    this['karakterff'] = this['karakterff'] ? this['karakterff'] : {};

    if (this.game[id] === 'karakterff' && id in this['karakterff']) {
        if (!m.text) return !0;

        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text.trim());
        if (isSurrender) {
            clearTimeout(this['karakterff'][id][2]);
            let ans = this['karakterff'][id][3] || this['karakterff'][id][1]?.result?.jawaban;
            let ansDisplay = Array.isArray(ans) ? ans.join(' / ') : ans;
            delete this['karakterff'][id];
            delete this.game[id];
            return m.reply(`🏳️ *Kamu Menyerah!*\n\nJawabannya adalah: *${ansDisplay}*`);
        }

        let rawAns = this['karakterff'][id][3] || this['karakterff'][id][1]?.result?.jawaban;
        ...
        if (isCorrect) {
            ...
            let extraDesc = this['karakterff'][id][1]?.result?.deskripsi ? `\n\n📝 *Penjelasan:* ${this['karakterff'][id][1].result.deskripsi}` : '';
            ...
            let isReply = m.quoted && m.quoted.id === this['karakterff'][id][0]?.key?.id;
            if (isReply) {
