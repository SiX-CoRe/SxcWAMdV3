...
handler.before = async function (m) {
    ...
    if (this.game[id] === 'siapakahaku' && id in this['siapakahaku']) {
        if (!m.text) return !0;

        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text.trim());
        if (isSurrender) {
            clearTimeout(this['siapakahaku'][id][2]);
            let ans = this['siapakahaku'][id][3] || this['siapakahaku'][id][1]?.result?.jawaban;
        ...
        let rawAns = this['siapakahaku'][id][3] || this['siapakahaku'][id][1]?.result?.jawaban;
        ...
        if (isCorrect) {
            ...
            let extraDesc = this['siapakahaku'][id][1]?.result?.deskripsi ? `\n\n📝 *Penjelasan:* ${this['siapakahaku'][id][1].result.deskripsi}` : '';
            ...
            let isReply = m.quoted && m.quoted.id === this['siapakahaku'][id][0]?.key?.id;
            if (isReply) {
