...
handler.before = async function (m) {
    ...
    if (this.game[id] === 'tebakjkt' && id in this['tebakjkt']) {
        if (!m.text) return !0;

        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text.trim());
        if (isSurrender) {
            clearTimeout(this['tebakjkt'][id][2]);
            let ans = this['tebakjkt'][id][3] || this['tebakjkt'][id][1]?.result?.jawaban;
        ...
        let rawAns = this['tebakjkt'][id][3] || this['tebakjkt'][id][1]?.result?.jawaban;
        ...
        if (isCorrect) {
            ...
            let extraDesc = this['tebakjkt'][id][1]?.result?.deskripsi ? `\n\n📝 *Penjelasan:* ${this['tebakjkt'][id][1].result.deskripsi}` : '';
            ...
            let isReply = m.quoted && m.quoted.id === this['tebakjkt'][id][0]?.key?.id;
            if (isReply) {
