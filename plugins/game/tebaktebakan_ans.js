...
handler.before = async function (m) {
    ...
    if (this.game[id] === 'tebaktebakan' && id in this['tebaktebakan']) {
        if (!m.text) return !0;

        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text.trim());
        if (isSurrender) {
            clearTimeout(this['tebaktebakan'][id][2]);
            let ans = this['tebaktebakan'][id][3] || this['tebaktebakan'][id][1]?.result?.jawaban;
        ...
        let rawAns = this['tebaktebakan'][id][3] || this['tebaktebakan'][id][1]?.result?.jawaban;
        ...
        if (isCorrect) {
            ...
            let extraDesc = this['tebaktebakan'][id][1]?.result?.deskripsi ? `\n\n📝 *Penjelasan:* ${this['tebaktebakan'][id][1].result.deskripsi}` : '';
            ...
            let isReply = m.quoted && m.quoted.id === this['tebaktebakan'][id][0]?.key?.id;
            if (isReply) {
