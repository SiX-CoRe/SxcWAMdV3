...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    if (id in conn['tebaklirik'] || id in conn.game) {
        conn.reply(m.chat, `⚠️ Masih ada game *${conn.game[id] || 'tebaklirik'}* yang belum terjawab di chat ini!\nSelesaikan atau ketik *nyerah* terlebih dahulu.`, conn['tebaklirik']?.[id]?.[0] || m);
        throw false;
    }
    ...
    try {
        ...
        const response = await fetch(`${global.web}/api/game/tebaklirik?apikey=${apiKey}`);
        ...
        console.error('[Game tebaklirik Error]', e);
