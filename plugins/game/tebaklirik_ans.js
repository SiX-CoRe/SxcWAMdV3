...
handler.before = async function (m) {
    ...
    if (this.game[id] === 'tebaklirik' && id in this['tebaklirik']) {
        if (!m.text) return !0;

        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text.trim());
        if (isSurrender) {
            clearTimeout(this['tebaklirik'][id][2]);
            let ans = this['tebaklirik'][id][3] || this['tebaklirik'][id][1]?.result?.jawaban;
        ...
        let rawAns = this['tebaklirik'][id][3] || this['tebaklirik'][id][1]?.result?.jawaban;
        ...
        if (isCorrect) {
            ...
            let extraDesc = this['tebaklirik'][id][1]?.result?.deskripsi ? `\n\n📝 *Penjelasan:* ${this['tebaklirik'][id][1].result.deskripsi}` : '';
            ...
            let isReply = m.quoted && m.quoted.id === this['tebaklirik'][id][0]?.key?.id;
            if (isReply) {
