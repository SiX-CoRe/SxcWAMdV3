...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    conn['surah'] = conn['surah'] ? conn['surah'] : {};
    let id = m.chat;

    if (id in conn['surah'] || id in conn.game) {
        conn.reply(m.chat, `⚠️ Masih ada game *${conn.game[id] || 'surah'}* yang belum terjawab di chat ini!\nSelesaikan atau ketik *nyerah* terlebih dahulu.`, conn['surah']?.[id]?.[0] || m);
        throw false;
    }
    ...
    try {
        ...
        const response = await fetch(`${global.web}/api/game/surah?apikey=${apiKey}`);
        ...
        let text = `🎮 *${'TEBAK AYAT & SURAH AL-QURAN'}*\n\n`;
        text += `📝 *Soal:* ${soalText}\n`;
        ...
        conn['surah'][id] = [
            ...
            json,
            setTimeout(() => {
                if (conn['surah'] && conn['surah'][id]) {
                    let ansDisplay = Array.isArray(answerData) ? answerData.join(' / ') : answerData;
                    conn.reply(m.chat, `⏳ *WAKTU HABIS!*\n\nJawabannya adalah: *${ansDisplay}*`, conn['surah'][id][0]);
                    delete conn['surah'][id];
        ...
        console.error('[Game surah Error]', e);
