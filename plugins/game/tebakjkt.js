...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    if (id in conn['tebakjkt'] || id in conn.game) {
        conn.reply(m.chat, `⚠️ Masih ada game *${conn.game[id] || 'tebakjkt'}* yang belum terjawab di chat ini!\nSelesaikan atau ketik *nyerah* terlebih dahulu.`, conn['tebakjkt']?.[id]?.[0] || m);
        throw false;
    }
    ...
    try {
        ...
        console.error('[Game tebakjkt Error]', e);
