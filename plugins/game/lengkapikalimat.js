...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    if (id in conn['lengkapikalimat'] || id in conn.game) {
        conn.reply(m.chat, `⚠️ Masih ada game *${conn.game[id] || 'lengkapikalimat'}* yang belum terjawab di chat ini!\nSelesaikan atau ketik *nyerah* terlebih dahulu.`, conn['lengkapikalimat']?.[id]?.[0] || m);
        throw false;
    }
    ...
    try {
        ...
        const response = await fetch(`${global.web}/api/game/lengkapikalimat?apikey=${apiKey}`);
        ...
        console.error('[Game lengkapikalimat Error]', e);
