...
handler.before = async function (m) {
    ...
    if (this.game[id] === 'tebakkalimat' && id in this['tebakkalimat']) {
        if (!m.text) return !0;

        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text.trim());
        if (isSurrender) {
            clearTimeout(this['tebakkalimat'][id][2]);
            let ans = this['tebakkalimat'][id][3] || this['tebakkalimat'][id][1]?.result?.jawaban;
        ...
        let rawAns = this['tebakkalimat'][id][3] || this['tebakkalimat'][id][1]?.result?.jawaban;
        ...
        if (isCorrect) {
            ...
            let extraDesc = this['tebakkalimat'][id][1]?.result?.deskripsi ? `\n\n📝 *Penjelasan:* ${this['tebakkalimat'][id][1].result.deskripsi}` : '';
            ...
            let isReply = m.quoted && m.quoted.id === this['tebakkalimat'][id][0]?.key?.id;
            if (isReply) {
