...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    if (id in conn['asahotak'] || id in conn.game) {
        conn.reply(m.chat, `⚠️ Masih ada game *${conn.game[id] || 'asahotak'}* yang belum terjawab di chat ini!\nSelesaikan atau ketik *nyerah* terlebih dahulu.`, conn['asahotak']?.[id]?.[0] || m);
        throw false;
    }
    ...
    try {
        ...
        const response = await fetch(`${global.web}/api/game/asahotak?apikey=${apiKey}`);
        ...
        console.error('[Game asahotak Error]', e);
