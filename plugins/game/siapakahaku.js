...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    if (id in conn['siapakahaku'] || id in conn.game) {
        conn.reply(m.chat, `⚠️ Masih ada game *${conn.game[id] || 'siapakahaku'}* yang belum terjawab di chat ini!\nSelesaikan atau ketik *nyerah* terlebih dahulu.`, conn['siapakahaku']?.[id]?.[0] || m);
        throw false;
    }
    ...
    try {
        ...
        const response = await fetch(`${global.web}/api/game/siapakahaku?apikey=${apiKey}`);
        ...
        console.error('[Game siapakahaku Error]', e);
