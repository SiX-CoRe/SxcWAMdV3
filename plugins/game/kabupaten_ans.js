...
handler.before = async function (m) {
    ...
    if (this.game[id] === 'kabupaten' && id in this['kabupaten']) {
        if (!m.text) return !0;

        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text.trim());
        if (isSurrender) {
            clearTimeout(this['kabupaten'][id][2]);
            let ans = this['kabupaten'][id][3] || this['kabupaten'][id][1]?.result?.jawaban;
            let ansDisplay = Array.isArray(ans) ? ans.join(' / ') : ans;
            delete this['kabupaten'][id];
            delete this.game[id];
            return m.reply(`🏳️ *Kamu Menyerah!*\n\nJawabannya adalah: *${ansDisplay}*`);
        }

        let rawAns = this['kabupaten'][id][3] || this['kabupaten'][id][1]?.result?.jawaban;
        ...
        if (isCorrect) {
            ...
            let extraDesc = this['kabupaten'][id][1]?.result?.deskripsi ? `\n\n📝 *Penjelasan:* ${this['kabupaten'][id][1].result.deskripsi}` : '';
            ...
            let isReply = m.quoted && m.quoted.id === this['kabupaten'][id][0]?.key?.id;
            if (isReply) {
