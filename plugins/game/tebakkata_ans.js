...
handler.before = async function (m) {
    ...
    if (this.game[id] === 'tebakkata' && id in this['tebakkata']) {
        if (!m.text) return !0;

        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text.trim());
        if (isSurrender) {
            clearTimeout(this['tebakkata'][id][2]);
            let ans = this['tebakkata'][id][3] || this['tebakkata'][id][1]?.result?.jawaban;
        ...
        let rawAns = this['tebakkata'][id][3] || this['tebakkata'][id][1]?.result?.jawaban;
        ...
        if (isCorrect) {
            ...
            let extraDesc = this['tebakkata'][id][1]?.result?.deskripsi ? `\n\n📝 *Penjelasan:* ${this['tebakkata'][id][1].result.deskripsi}` : '';
            ...
            let isReply = m.quoted && m.quoted.id === this['tebakkata'][id][0]?.key?.id;
            if (isReply) {
