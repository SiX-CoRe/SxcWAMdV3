...
handler.before = async function (m) {
    ...
    this['maths'] = this['maths'] ? this['maths'] : {};

    if (this.game[id] === 'maths' && id in this['maths']) {
        if (!m.text) return !0;

        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text.trim());
        if (isSurrender) {
            clearTimeout(this['maths'][id][2]);
            let ans = this['maths'][id][3] || this['maths'][id][1]?.result?.jawaban;
        ...
        let rawAns = this['maths'][id][3] || this['maths'][id][1]?.result?.jawaban;
        ...
        if (isCorrect) {
            ...
            let extraDesc = this['maths'][id][1]?.result?.deskripsi ? `\n\n📝 *Penjelasan:* ${this['maths'][id][1].result.deskripsi}` : '';
            ...
            let isReply = m.quoted && m.quoted.id === this['maths'][id][0]?.key?.id;
            if (isReply) {
