...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    if (id in conn['tebaktebakan'] || id in conn.game) {
        conn.reply(m.chat, `⚠️ Masih ada game *${conn.game[id] || 'tebaktebakan'}* yang belum terjawab di chat ini!\nSelesaikan atau ketik *nyerah* terlebih dahulu.`, conn['tebaktebakan']?.[id]?.[0] || m);
        throw false;
    }
    ...
    try {
        ...
        const response = await fetch(`${global.web}/api/game/tebaktebakan?apikey=${apiKey}`);
        ...
        console.error('[Game tebaktebakan Error]', e);
